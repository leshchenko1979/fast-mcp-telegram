...
async def send_message_to_phone_impl(
    ...
    try:
        ...
        try:
            ...
            logger.debug(f"Contact {phone_number} already exists, using existing contact")
        except Exception:
            ...
            if not result.users:
                ...
                log_operation_error(request_id, "adding contact", ValueError(error_msg), params)
        ...
        if remove_if_new and contact_was_new:
            try:
                ...
                logger.debug(f"Newly created contact {phone_number} removed after sending message")
            except Exception as e:
                logger.warning(f"Failed to remove newly created contact {phone_number}: {e}")
        elif remove_if_new and not contact_was_new:
            logger.debug(f"Contact {phone_number} was existing, not removing (remove_if_new=True but contact was not new)")
