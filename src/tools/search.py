...
def _matches_chat_type(entity, chat_type: str) -> bool:
    ...
    return (
        (chat_type == 'private' and entity_class == 'User') or
        (chat_type == 'group' and entity_class == 'Chat') or
        (chat_type == 'channel' and entity_class in ['Channel', 'ChannelForbidden'])
    )
...
async def _process_message_for_results(
    ...
    """
    if not message or not (hasattr(message, 'message') and message.message) or (hasattr(message, 'text') and message.text):
        return False
    ...
    try:
        ...
        if chat_id:
            ...
            try:
                ...
                await _execute_parallel_searches(search_tasks, collected, seen_keys, offset, limit)
        ...
        window = collected[offset:offset + limit] if limit is not None else collected[offset:]

        logger.info(f"[{request_id}] Found {len(window)} messages matching query: {query}")
...
async def _search_chat_messages(client, entity, query, limit, offset, chat_type, auto_expand_batches):
    ...
    while batch_count < max_batches and len(results) < limit:
        ...
        for message in batch:
            if await _process_message_for_results(client, message, entity, chat_type, results, limit):
    ...
    while batch_count < max_batches and len(results) < limit:
        ...
        for message in result.messages:
            try:
                ...
                if await _process_message_for_results(client, message, chat, chat_type, results, limit):
                    if len(results) >= limit:
                        break
            except Exception as e:
                logger.warning(f"Error processing message: {e}")
