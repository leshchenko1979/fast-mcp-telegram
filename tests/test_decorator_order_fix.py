...
class TestDecoratorOrderFix:
    ...
    async def test_with_auth_context_executes_with_valid_token(self):
        ...
        with patch("src.server.DISABLE_AUTH", False), \
            ...
            assert result["token_was_set"] is True, "Token should have been set in context"
            assert result["token_value"] == test_token, f"Expected {test_token}, got {result['token_value']}"
            assert result["used_provided_token"] is True, "Should use the provided token, not fall back to default"
    ...
    async def test_no_fallback_to_default_session_when_token_provided(self):
        ...
        with patch("src.server.DISABLE_AUTH", False), \
            ...
            assert result["used_provided_token"] is True, "Should use the provided token"
    ...
    async def test_stdio_mode_allows_fallback(self):
        ...
        with patch("src.server.DISABLE_AUTH", False), \
            ...
            assert result["fell_back"] is True, "Should fall back to default session in stdio mode"
    ...
    def test_decorator_order_verification(self):
        ...
        assert isinstance(search_contacts, FunctionTool), "search_contacts should be a FunctionTool"
        assert isinstance(send_or_edit_message, FunctionTool), "send_or_edit_message should be a FunctionTool"
        assert isinstance(read_messages, FunctionTool), "read_messages should be a FunctionTool"
        
        print("✅ All tool functions have correct decorator order (FastMCP FunctionTool)")
        ...
        with patch("src.server.DISABLE_AUTH", False), \
            ...
            async def simulate_tool_function():
                ...
                return {
                    ...
                    "issue_reproduced": current_token is None,  # This should be False after fix
                    "fix_working": current_token == user_token  # This should be True after fix
                }
            
            result = await simulate_tool_function()
            
            # Verify the fix is working
            assert result["issue_reproduced"] is False, "Original issue should be fixed - no fallback to default"
            assert result["fix_working"] is True, "Fix should be working - using provided token"
            assert result["context_token"] == result["user_provided_token"], "Context token should match user provided token"
            
            print("✅ Original issue has been fixed - no fallback to default session when token is provided")
